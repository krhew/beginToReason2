...
from tutor.py_helper_functions.mutation import reverse_mutate, mutate
...
def tutor(request):
    ...
    if request.method == 'POST':
        ...
        if user_auth(request):
            ...
            print("status: ", status)
            submitted_answer = reverse_mutate(json.loads(request.body.decode('utf-8'))['answer'].replace(" ", ""))
